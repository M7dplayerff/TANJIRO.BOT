const handler = async (m, { conn }) => {
  ...
  const loliList = global.loli;
  const url = loliList[Math.floor(Math.random() * loliList.length)];
  const caption = '*🌸😋 لــولــي 😋🌸*';

  conn.sendMessage(m.chat, { image: { url: url }, caption: caption }, { quoted: m });
};
