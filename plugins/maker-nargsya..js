...
const handler = async (m, { conn, args }) => {
    ...
    if (args.length >= 1) {
        ...
        throw "- *🔱 رد عــلــي نــص او اكــتــب نــص بــعــد الــامــر لـتـفـعـيــل الـتـأثــيــر*";
    }
await conn.sendMessage(m.chat, { react: { text: "🤡",key: m.key,}
  })
    if (!text) return m.reply('- *🔱 ادخــــل نـــــص*');
    if (text.length > 2000) return m.reply('الحد الأقصى - *🔱 الــحــد الــاقـصـي الـفـيـن حـــرف*');
    ...
    try {
        ...
        m.reply('- *🔱 حـــدث خـــطـــأ*');
    }
};

handler.help = ['نرجسيه'];
handler.tags = ['ma'];
handler.command = /^(نرجسيه|نرجسية)$/i;
