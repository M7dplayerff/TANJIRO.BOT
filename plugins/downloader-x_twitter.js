...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) throw `- *🔱ادخــل رابــط فـيـديــو او صــوت او صــورة تــويـتـر*`;
  ...
  try {
    await conn.sendMessage(m.chat, {text: '- *🔱 جــاري الـمـعـالــجــة*'}, {quoted: m}); 
    const res = await TwitterDL(text);
    if (res?.result.type == 'video') {
      const caption = res?.result.caption ? res.result.caption : '- *🔱 لـا يــوجــد وصــف*';
    ...
    enviando = false;
    throw '- *🔱 فــشــل الــتــحــمــيــل*';
    return;
  }
};
hanler.help = ['تنزيل_تويتر']
handler.tags = ['do']
handler.command = /^((تنزيل_تويتر|xdl|dlx|twdl|tw|twt|twitter)(dl)?)$/i;
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message: '- *🔱 لــم يــتــم الـعـثـور عــلــي تـغـريــدة*',
      });
